...
const Navigation: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 transition-colors duration-300">
      <div className="flex justify-around py-3 items-center flex-wrap">
        <Link to="/" className={`flex flex-col items-center transition-colors duration-300 ${isActive('/') ? 'text-[#1b76f5]' : 'text-gray-600 dark:text-gray-400'} px-3 mb-1`}>
          {isActive('/') ? <HomeSolidIcon className="w-6 h-6" /> : <HomeIcon className="w-6 h-6" />}
          <span className="text-xs">Beranda</span>
        </Link>
        <Link to="/last-read" className={`flex flex-col items-center transition-colors duration-300 ${isActive('/last-read') ? 'text-[#1b76f5]' : 'text-gray-600 dark:text-gray-400'} px-3 mb-1`}>
        ...
        <Link to="/favorites" className={`flex flex-col items-center transition-colors duration-300 ${isActive('/favorites') ? 'text-[#1b76f5]' : 'text-gray-600 dark:text-gray-400'} px-3 mb-1`}>
          {isActive('/favorites') ? <HeartSolidIcon className="w-6 h-6" /> : <HeartIcon className="w-6 h-6" />}
          <span className="text-xs">Favorit</span>
        </Link>
        <Link to="/settings" className={`flex flex-col items-center transition-colors duration-300 ${isActive('/settings') ? 'text-[#1b76f5]' : 'text-gray-600 dark:text-gray-400'} px-3 mb-1`}>
